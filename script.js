...
const el = {
  ...
  dialAqi: document.getElementById('dial-aqi'),
...
function aqiColor(aqi) {
  if (aqi <= 50) return '#6FA8C7';   // Good
  if (aqi <= 100) return '#C9A227';  // Moderate
  if (aqi <= 150) return '#C77452';  // Unhealthy for sensitive groups
  return '#A0396B';                  // Unhealthy and worse
}

function getAqiAt(isoTime) {
  if (!state.airQuality || !state.airQuality.hourly) return null;
  const idx = state.airQuality.hourly.time.indexOf(isoTime);
  if (idx < 0) return null;
  const val = state.airQuality.hourly.us_aqi[idx];
...
async function fetchAirQuality(place) {
  try {
    ...
    return null; // AQI is supplementary — don't break the main forecast over it.
  }
}
...
function renderAdvisories(globalIdx) {
...
function renderTrace() {
  ...
  const w = 600, h = 120;
  const padLeft = 36, padRight = 10, padTop = 12, padBottom = 12;
  ...
  state.trace = {
    ...
    min, max, w, h, padLeft, padRight, padTop, padBottom,
  };
...
function drawYAxis() {
  const { min, max, w, h, padLeft, padRight, padTop, padBottom } = state.trace;
  const mid = (min + max) / 2;
  const levels = [
    { value: max, y: padTop },
    { value: mid, y: h / 2 },
    { value: min, y: h - padBottom },
  ];

  return levels.map(({ value, y }) => {
    const label = displayTemp(value) + '°';
    return `
      <line x1="${padLeft}" y1="${y.toFixed(1)}" x2="${w - padRight}" y2="${y.toFixed(1)}" stroke="#2B3542" stroke-width="1" stroke-dasharray="2 3"/>
      <text x="${padLeft - 8}" y="${y.toFixed(1)}" text-anchor="end" dominant-baseline="middle" font-family="IBM Plex Mono, monospace" font-size="9" fill="#8B94A0">${label}</text>
    `;
  }).join('');
}
...
function moveScrubberTo(idx) {
  ...
  const aqi = getAqiAt(trace.times[idx]);
  el.dialAqi.textContent = aqi != null ? Math.round(aqi) : '—';
  el.dialAqi.style.color = aqi != null ? aqiColor(aqi) : '';
}
