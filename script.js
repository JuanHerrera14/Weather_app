...
async function loadWeather(place) {
  ...
  try {
    const params = new URLSearchParams({
      ...
      hourly: 'temperature_2m,relative_humidity_2m,precipitation,precipitation_probability,wind_speed_10m,wind_direction_10m,uv_index',
...
function render() {
  ...
  // Humidity / wind / precip dials are driven by the trace scrubber
  // (see moveScrubberTo), so they start at "now" and update as it's dragged.
...
function renderAdvisories() {
  ...
  const rainKey = maxRainProb >= 50 ? 'umbrella' : 'umbrella-off';

  el.rainIcon.innerHTML = ICONS[rainKey];
  el.rainVerdict.textContent = `${Math.round(maxRainProb)}%`;
  el.rainDetail.textContent = maxRainProb > 0
    ? `Peak chance around ${hourLabels[peakIdx]}, next ${HOURS_AHEAD} hours.`
    : `No rain in the forecast for the next ${HOURS_AHEAD} hours.`;
  barChart(el.rainChart, rainProbs, 100, '#6FA8C7');

  // --- UV index ---
  const uvMax = daily.uv_index_max ? daily.uv_index_max[0] : 0;
  let uvLabel;
  if (uvMax < 3) uvLabel = 'Low';
  else if (uvMax < 6) uvLabel = 'Moderate';
  else if (uvMax < 8) uvLabel = 'High';
  else if (uvMax < 11) uvLabel = 'Very high';
  else uvLabel = 'Extreme';

  el.sunIcon.innerHTML = ICONS.sunscreen;
  el.sunVerdict.textContent = uvMax.toFixed ? uvMax.toFixed(1) : uvMax;
  el.sunDetail.textContent = `${uvLabel}, peak value expected today.`;
  ...
  const windPeakIdx = windAhead.indexOf(Math.max(...windAhead));

  el.windIcon.innerHTML = ICONS.windIcon;
  el.windVerdict.textContent = `${Math.round(maxWind)} km/h`;
  el.windDetail.textContent = `Currently ${Math.round(windNow)} km/h, peaking near ${hourLabels[windPeakIdx]}.`;
...
function moveScrubberTo(idx) {
  ...
  // Drive the humidity / wind / precip dials from the same selected hour.
  const humidity = hourly.relative_humidity_2m[globalIdx];
  const precip = hourly.precipitation[globalIdx];
  const windDir = hourly.wind_direction_10m[globalIdx];

  el.dialHumidity.textContent = (humidity != null ? Math.round(humidity) : '—') + '%';
  el.dialPrecip.textContent = (precip != null ? precip : '—') + 'mm';
  el.dialWind.textContent = (wind != null ? Math.round(wind) : '—') + ' km/h' + (windDir != null ? ' ' + windDir16(windDir) : '');
  if (windDir != null) el.windArrow.setAttribute('transform', `rotate(${windDir} 20 20)`);
}
