...
let state = {
  ...
  trace: null,       // { startIdx, times, points } for the scrub slider
};
...
const el = {
  ...
  scrubRain: document.getElementById('scrub-rain'),
  scrubUv: document.getElementById('scrub-uv'),
  scrubWind: document.getElementById('scrub-wind'),
};
...
// ---------- Advisories: rain / sun / wind, next few hours ----------

function nowIndex(times) {
  const nowTime = new Date(state.weather.current.time).getTime();
  let idx = times.findIndex(t => new Date(t).getTime() >= nowTime);
  return idx < 0 ? 0 : idx;
}
...
function renderAdvisories() {
  const { hourly, current, daily } = state.weather;
  const HOURS_AHEAD = 6;
  const start = nowIndex(hourly.time);
  const hourLabels = hourly.time.slice(start, start + HOURS_AHEAD).map(t =>
    new Date(t).toLocaleTimeString([], { hour: 'numeric' })
  );

  // --- Rain / umbrella ---
  const rainProbs = hourly.precipitation_probability.slice(start, start + HOURS_AHEAD);
  const maxRainProb = rainProbs.length ? Math.max(...rainProbs) : 0;
  const peakIdx = rainProbs.indexOf(maxRainProb);

  let rainVerdict, rainDetail, rainKey;
  if (maxRainProb >= 50) {
    rainVerdict = 'Bring it';
    rainDetail = `${maxRainProb}% chance of rain around ${hourLabels[peakIdx]}.`;
    rainKey = 'umbrella';
  } else if (maxRainProb >= 20) {
    rainVerdict = 'Maybe pack it';
    rainDetail = `${maxRainProb}% chance of rain in the next ${HOURS_AHEAD} hours.`;
    rainKey = 'umbrella';
  } else {
    rainVerdict = 'Leave it home';
    rainDetail = `Only ${maxRainProb}% chance of rain in the next ${HOURS_AHEAD} hours.`;
    rainKey = 'umbrella-off';
  }
  el.rainIcon.innerHTML = ICONS[rainKey];
  el.rainVerdict.textContent = rainVerdict;
  el.rainDetail.textContent = rainDetail;
  barChart(el.rainChart, rainProbs, 100, '#6FA8C7');

  // --- Sun / sunscreen ---
  const uvMax = daily.uv_index_max ? daily.uv_index_max[0] : 0;
  let uvLabel, sunVerdict, sunDetail;
  if (uvMax < 3) { uvLabel = 'Low'; sunVerdict = 'Optional'; }
  else if (uvMax < 6) { uvLabel = 'Moderate'; sunVerdict = 'Wear it'; }
  else if (uvMax < 8) { uvLabel = 'High'; sunVerdict = 'Wear it'; }
  else if (uvMax < 11) { uvLabel = 'Very high'; sunVerdict = 'Wear it, reapply'; }
  else { uvLabel = 'Extreme'; sunVerdict = 'Wear it, seek shade'; }
  sunDetail = `Peak UV index today: ${uvMax.toFixed ? uvMax.toFixed(1) : uvMax} (${uvLabel}).`;

  el.sunIcon.innerHTML = ICONS.sunscreen;
  el.sunVerdict.textContent = sunVerdict;
  el.sunDetail.textContent = sunDetail;
  ...
  const windAhead = hourly.wind_speed_10m.slice(start, start + HOURS_AHEAD);
  const maxWind = Math.max(windNow, ...windAhead);

  let windVerdict, windDetail;
  if (maxWind < 12) {
    windVerdict = 'No jacket needed';
    windDetail = `Winds stay calm, up to ${Math.round(maxWind)} km/h.`;
  } else if (maxWind < 25) {
    windVerdict = 'Light layer';
    windDetail = `Breezy later, up to ${Math.round(maxWind)} km/h.`;
  } else if (maxWind < 40) {
    windVerdict = 'Bring a jacket';
    windDetail = `Windy, gusting up to ${Math.round(maxWind)} km/h.`;
  } else {
    windVerdict = 'Wear a windbreaker';
    windDetail = `Very windy, up to ${Math.round(maxWind)} km/h.`;
  }
  el.windIcon.innerHTML = ICONS.windIcon;
  el.windVerdict.textContent = windVerdict;
  el.windDetail.textContent = windDetail;
  barChart(el.windChart, [windNow, ...windAhead], Math.max(20, maxWind), '#C9A227');
}
...
function renderTrace() {
  ...
  state.trace = { startIdx, times: sliceTimes, points, length: slice.length };
  ...
  el.traceSvg.innerHTML = `
    ...
    <line id="trace-marker-line" x1="${points[0][0]}" y1="0" x2="${points[0][0]}" y2="${h}" />
    <circle id="trace-marker-dot" cx="${points[0][0]}" cy="${points[0][1]}" r="4"/>
  `;
...
function moveScrubberTo(idx) {
  ...
  if (!trace || !trace.points[idx]) return;

  const [x, y] = trace.points[idx];
  ...
  const rain = hourly.precipitation_probability[globalIdx];
  const uv = hourly.uv_index[globalIdx];
  const wind = hourly.wind_speed_10m[globalIdx];

  el.scrubRain.textContent = (rain != null ? Math.round(rain) : '—') + '%';
  el.scrubUv.textContent = uv != null ? uv.toFixed(1) : '—';
  el.scrubWind.textContent = (wind != null ? Math.round(wind) : '—') + ' km/h';
}
